...
`transpose()` swaps the order of nested monad types, converting between `Option<Result<T,E>>` and `Result<Option<T>,E>` (or vice versa). This is essential when working with operations that can both fail and return optional values.
...
**When to use:** When you have nested monads and need to change the order of operations or error handling precedence.
