...
describe('None', function (): void {
    describe('Happy Paths', function (): void {
        ...
        test('returns result of fallback function when calling unwrapOrElse on None', function (): void {
            $none = None::create();
            expect($none->unwrapOrElse(fn (): string => 'foo'))->toBe('foo');
        });
        ...
        test('returns alternative Option when calling orElse on None', function (): void {
            ...
            expect(None::create()->orElse(fn (): Some => $option))->toBe($option);
        });
        ...
        test('does not execute map function and returns self when mapping None', function (): void {
            ...
            expect($none->map(function (): void {
                throw TestException::shouldNotBeCalled();
            }))->toBe($none);
        });
        ...
        test('does not execute inspect function and returns self when inspecting None', function (): void {
            $none = None::create();

            expect($none->inspect(function (): void {
                throw TestException::shouldNotBeCalled();
            }))->toBe($none);
        });

        test('returns self when calling cloned on None', function (): void {
            $none = None::create();

            expect($none->cloned())->toBe($none);
        });
    });
