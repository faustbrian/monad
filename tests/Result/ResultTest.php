...
describe('Result', function (): void {
    describe('Happy Paths', function (): void {
        ...
        test('converts Option to Result with error fallback using okOr and okOrElse', function (): void {
            ...
            $r3 = $none->okOrElse(fn (): string => 'gen');
            expect($r3->isErr())->toBeTrue();
            expect($r3->unwrapErr())->toBe('gen');

            // Some returns Ok without calling error function
            $r4 = $some->okOrElse(fn (): string => throw TestException::shouldNotBeCalled());
            expect($r4->isOk())->toBeTrue();
            expect($r4->unwrap())->toBe('x');
        });
        ...
        test('performs logical operations and unwraps values with fallbacks', function (): void {
            ...
            // orElse
            $r = $err->orElse(fn (string $e): Ok => new Ok(mb_strlen($e)));
            expect($r->isOk())->toBeTrue();
            expect($r->unwrap())->toBe(1);

            // orElse on Ok returns itself without calling function
            $okResult = $ok->orElse(fn ($e): Err => throw TestException::shouldNotBeCalled());
            expect($okResult)->toBe($ok);

            // unwrapOr / unwrapOrElse
            expect($ok->unwrapOr(0))->toBe(1);
            expect($err->unwrapOr(7))->toBe(7);
            expect($err->unwrapOrElse(fn (string $e): int => mb_strlen($e) + 4))->toBe(5);
        });
        ...
        test('throws RuntimeException when transposing Ok with non-Option value', function (): void {
            $this->expectException(RuntimeException::class);
            $this->expectExceptionMessage('Result::transpose expects Ok(Option), got');

            $invalidOk = new Ok('not-an-option');
            $invalidOk->transpose();
        });
    ...
    describe('Sad Paths', function (): void {
        ...
        test('throws RuntimeException when calling unwrap on Err', function (): void {
            $this->expectException(RuntimeException::class);
            $this->expectExceptionMessage('Called unwrap() on Err');

            $err = new Err('error value');
            $err->unwrap();
        });

        test('throws RuntimeException when calling unwrapErr on Ok', function (): void {
            $this->expectException(RuntimeException::class);
            $this->expectExceptionMessage('Called unwrapErr() on Ok');

            $ok = new Ok('success value');
            $ok->unwrapErr();
        });
    });
});
