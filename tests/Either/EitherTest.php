...
describe('Either', function (): void {
    describe('Happy Paths', function (): void {
        ...
        test('creates Either from callable using tryCatch', function (): void {
            $success = Either::tryCatch(fn () => 'result');
        ...
        test('chains Either-returning operations with flatMap', function (): void {
            ...
            $toLeft = $right->flatMap(fn (int $x) => new Left('failed'));
            expect($toLeft->isLeft())->toBeTrue();
            expect($toLeft->unwrapLeft())->toBe('failed');

            $leftResult = $left->flatMap(fn ($x) => new Right($x));
        ...
        test('chains Either-returning operations with andThen alias', function (): void {
            ...
            $result = $right->andThen(fn (int $x) => new Right($x + 2));
            expect($result->isRight())->toBeTrue();
        ...
        test('filters Right value based on predicate', function (): void {
            ...
            $left = new Left('error');
            $leftFiltered = $left->filter(fn ($x) => true, 'never');
        ...
        test('returns Right value with unwrapOrElse computed fallback', function (): void {
            ...
            expect($right->unwrapOrElse(fn ($e) => 0))->toBe(10);
            expect($left->unwrapOrElse(fn (string $e) => mb_strlen($e)))->toBe(5);
        });
        ...
        test('checks Right state with predicate using isRightAnd', function (): void {
            ...
            expect($left->isRightAnd(fn ($x) => true))->toBeFalse();
        });
        ...
        test('checks Left state with predicate using isLeftAnd', function (): void {
            ...
            expect($left->isLeftAnd(fn (string $e) => str_starts_with($e, 'err')))->toBeTrue();
            expect($left->isLeftAnd(fn (string $e) => str_starts_with($e, 'warn')))->toBeFalse();
            expect($right->isLeftAnd(fn ($e) => true))->toBeFalse();
        });
    ...
    describe('Edge Cases', function (): void {
        ...
        test('filter converts Right to Left on predicate failure', function (): void {
            ...
            $result = $right
                ->filter(fn (int $x) => $x > 5, 'too small')
                ->filter(fn (int $x) => $x < 20, 'too large');

            expect($result->isRight())->toBeTrue();

            $failed = $right->filter(fn (int $x) => $x > 20, 'too small');
        ...
        test('chained transformations preserve Either type', function (): void {
            $result = new Right(5)
                ...
                ->map(fn (int $x) => $x - 1)
                ->filter(fn (int $x) => $x > 10, 'too small');
        ...
        test('handles mixed type transformations', function (): void {
            $result = new Right(5)
                ...
                ->map(fn (string $s) => str_repeat($s, 2))
    ...
    describe('Regressions', function (): void {
        test('Left operations never affect Right side', function (): void {
            ...
            $result = $original
                ->mapLeft(fn (string $e) => mb_strtoupper($e))
                ->map(fn ($x) => 'should never execute')
                ->flatMap(fn ($x) => new Right('should never execute'))
                ->filter(fn ($x) => false, 'irrelevant');
        ...
        test('Right operations never affect Left side', function (): void {
            ...
            $result = $original
                ->map(fn (int $x) => $x * 2)
                ->mapLeft(fn ($e) => 'should never execute')
                ->forLeft(fn ($e) => throw new RuntimeException('should not execute'))
                ->filter(fn (int $x) => $x === 20, 'would fail');
        ...
        test('ensures type consistency through transformations', function (): void {
            ...
            $rightResult = $right
                ->map(fn (int $x) => $x)
                ->flatMap(fn (int $x) => new Right($x))
                ->filter(fn (int $x) => true, 'fail');
        ...
        test('isRightAnd and isLeftAnd handle falsy predicate returns correctly', function (): void {
            ...
            expect($right->isRightAnd(fn (int $x) => $x))->toBeFalse();
            expect($left->isLeftAnd(fn (int $e) => $e))->toBeFalse();

            // Predicate returns true
            expect($right->isRightAnd(fn (int $x) => true))->toBeTrue();
            expect($left->isLeftAnd(fn (int $e) => true))->toBeTrue();

            // Predicate returns 1 (truthy)
            expect($right->isRightAnd(fn (int $x) => 1))->toBeTrue();
            expect($left->isLeftAnd(fn (int $e) => 1))->toBeTrue();
        });
        ...
        test('filter preserves strict boolean comparison', function (): void {
            ...
            $result1 = $right->filter(fn (string $x) => 1, 'fail');
            expect($result1->isLeft())->toBeTrue();

            // Predicate returns exactly true
            $result2 = $right->filter(fn (string $x) => true, 'fail');
            expect($result2->isRight())->toBeTrue();

            // Predicate returns exactly false
            $result3 = $right->filter(fn (string $x) => false, 'fail');
        ...
        test('unwrapOrElse receives correct Left value', function (): void {
            ...
            $result = $left->unwrapOrElse(function ($e) use (&$receivedError) {
                $receivedError = $e;
        ...
        test('flatMap properly chains with mixed Left and Right results', function (): void {
            $result = new Right(5)
                ->flatMap(fn (int $x) => $x > 3 ? new Right($x * 2) : new Left('too small'))
                ->flatMap(fn (int $x) => $x > 20 ? new Left('too large') : new Right($x + 5));
            ...
            $failure = new Right(2)
                ->flatMap(fn (int $x) => $x > 3 ? new Right($x * 2) : new Left('too small'))
                ->flatMap(fn (int $x) => new Right($x + 5));
    ...
    describe('Map with Defaults', function (): void {
        describe('Happy Paths', function (): void {
            test('mapOr transforms Right value with function', function (): void {
                ...
                $result = $right->mapOr(0, fn (int $x) => $x * 2);
        ...
        describe('Edge Cases', function (): void {
            ...
            test('mapOr with expensive default computation', function (): void {
                ...
                $result = $right->mapOr(expensiveComputation(), fn (int $x) => $x * 2);
            ...
            test('mapOrElse receives correct Left value in default function', function (): void {
                ...
                $result = $left->mapOrElse(
                    function ($err) use (&$receivedError) {
                        $receivedError = $err;
        ...
        describe('Regressions', function (): void {
            test('mapOr does not evaluate function on Left', function (): void {
                ...
                $result = $left->mapOr(100, function ($x) use (&$evaluated) {
                    $evaluated = true;
    ...
    describe('Zip Operations', function (): void {
        describe('Happy Paths', function (): void {
            ...
            test('zipWith combines two Right Eithers with function', function (): void {
                ...
                $result = $right1->zipWith($right2, fn (int $a, int $b) => $a + $b);
        ...
        describe('Sad Paths', function (): void {
            ...
            test('zipWith returns first Left when first Either is Left', function (): void {
                ...
                $result = $left->zipWith($right, fn ($a, $b) => $a + $b);
            ...
            test('zipWith returns second Left when first is Right and second is Left', function (): void {
                ...
                $result = $right->zipWith($left, fn ($a, $b) => $a + $b);
        ...
        describe('Edge Cases', function (): void {
            ...
            test('zipWith with string concatenation', function (): void {
                ...
                $result = $right1->zipWith($right2, fn (string $a, string $b) => "{$a} {$b}");
            ...
            test('zipWith does not call function when either Either is Left', function (): void {
                ...
                $result = $left->zipWith($right, function ($a, $b) use (&$called) {
                    $called = true;
            ...
            test('chaining zip operations', function (): void {
                ...
                $result = $right1
                    ->zip($right2)
                    ->zipWith($right3, fn (array $tuple, int $third) => [...$tuple, $third]);
        ...
        describe('Regressions', function (): void {
            ...
            test('zipWith short-circuits on first Left without calling function', function (): void {
                ...
                $result = $left->zipWith($right, function ($a, $b) use (&$functionCalled) {
                    $functionCalled = true;
    ...
    describe('Collection Operations', function (): void {
        describe('Happy Paths', function (): void {
            ...
            test('traverse maps and sequences array items', function (): void {
                ...
                $result = Either::traverse($items, fn (int $x) => new Right($x * 2));
            ...
            test('traverse with empty array', function (): void {
                $result = Either::traverse([], fn ($x) => new Right($x));
        ...
        describe('Sad Paths', function (): void {
            ...
            test('traverse returns first Left on failure', function (): void {
                ...
                $result = Either::traverse($items, function (int $x) {
                    return $x === 3 ? new Left('failed at 3') : new Right($x * 2);
                });
        ...
        describe('Edge Cases', function (): void {
            ...
            test('traverse with validation logic', function (): void {
                ...
                $result = Either::traverse($emails, function (string $email) {
            ...
            test('traverse with complex transformations', function (): void {
                ...
                $result = Either::traverse($users, function (array $user) {
                    return isset($user['id'], $user['name'])
        ...
        describe('Regressions', function (): void {
            ...
            test('traverse stops at first Left without processing remaining items', function (): void {
                ...
                $result = Either::traverse($items, function (int $x) use (&$processedCount) {
                    $processedCount++;
