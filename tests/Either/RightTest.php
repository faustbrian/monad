...
describe('Right', function (): void {
    describe('Happy Paths', function (): void {
        ...
        test('returns contained value without calling unwrapOrElse', function (): void {
            ...
            $result = $right->unwrapOrElse(function ($e) use (&$called): string {
                $called = true;
        ...
        test('applies map transformation', function (): void {
            ...
            $result = $right->map(fn (int $x): int => $x * 2);
        ...
        test('applies only right function in bimap', function (): void {
            ...
            $result = $right->bimap(
                ...
                function (int $r) use (&$rightCalled): int {
                    $rightCalled = true;
        ...
        test('executes flatMap operation', function (): void {
            ...
            $toLeft = $right->flatMap(fn (int $x): Left => new Left('error'));
        ...
        test('checks right state with predicate', function (): void {
            ...
            expect($right->isLeftAnd(fn ($l): true => true))->toBeFalse();
        });
    ...
    describe('Edge Cases', function (): void {
        ...
        test('chains flatMap transformations', function (): void {
            ...
            $result = $right
                ->flatMap(fn (int $x): Right => new Right($x * 2))
                ->flatMap(fn (int $x): Right => new Right($x + 1))
                ->flatMap(fn (int $x): Right => new Right((string) $x));
        ...
        test('filter with strict true comparison', function (): void {
            ...
            $strictTrue = $right->filter(fn (int $x): true => true, 'fail');
            expect($strictTrue->isRight())->toBeTrue();

            // Returns truthy value (not exactly true)
            $truthyValue = $right->filter(fn (int $x): int => 1, 'fail');
            expect($truthyValue->isLeft())->toBeTrue();

            // Returns false
            $falseValue = $right->filter(fn (int $x): false => false, 'fail');
        ...
        test('flatMap chain that transitions to Left', function (): void {
            $result = new Right(5)
                ->flatMap(fn (int $x): Right => new Right($x * 2))
                ->flatMap(fn (int $x): Left|Right => $x > 8 ? new Left('too large') : new Right($x))
                ->flatMap(fn (int $x): Right => new Right($x + 100)); // Never executed
    ...
    describe('Regressions', function (): void {
        ...
        test('unwrapOrElse does not call callable when Right', function (): void {
            ...
            $result = $right->unwrapOrElse(function () use (&$called): string {
                $called = true;
        ...
        test('forAll always returns same instance', function (): void {
            ...
            $result = $right->forAll(fn ($x): null => null);
        ...
        test('inspect always returns same instance', function (): void {
            ...
            $result = $right->inspect(fn ($x): null => null);
        ...
        test('bimap only transforms Right value', function (): void {
            ...
            $result = $right->bimap(
                fn ($l) => throw TestException::shouldNotBeCalled(),
                fn (int $r): int => $r * 3,
            );
        ...
        test('isRightAnd converts return value to boolean', function (): void {
            ...
            expect($right->isRightAnd(fn ($r): int => 0))->toBeFalse();
            expect($right->isRightAnd(fn ($r): int => 1))->toBeTrue();
            expect($right->isRightAnd(fn ($r): string => ''))->toBeFalse();
            expect($right->isRightAnd(fn ($r): string => 'truthy'))->toBeTrue();
            expect($right->isRightAnd(fn ($r): null => null))->toBeFalse();
        });
        ...
        test('flatMap properly propagates types', function (): void {
            $result = new Right(5)
                ->flatMap(fn (int $x): Right => new Right((string) $x))
                ->flatMap(fn (string $x): Right => new Right([$x]))
                ->flatMap(fn (array $x): Right => new Right(count($x)));
