...
describe('Left', function (): void {
    describe('Happy Paths', function (): void {
        ...
        test('computes fallback value with unwrapOrElse', function (): void {
            ...
            $result = $left->unwrapOrElse(fn (string $e) => "Handled: {$e}");
            expect($result)->toBe('Handled: error');

            $lengthResult = $left->unwrapOrElse(fn (string $e) => mb_strlen($e));
        ...
        test('skips flatMap operation', function (): void {
            ...
            $result = $left->flatMap(function ($value) use (&$called) {
                $called = true;
        ...
        test('ignores filter operation', function (): void {
            ...
            $result = $left->filter(function ($value) use (&$called) {
                $called = true;
        ...
        test('executes left branch in match', function (): void {
            ...
            $result = $left->match(
                function (string $e) use (&$leftCalled) {
                    $leftCalled = true;
        ...
        test('checks left state with predicate', function (): void {
            ...
            expect($left->isLeftAnd(fn (string $e) => mb_strlen($e) === 5))->toBeTrue();
            expect($left->isLeftAnd(fn (string $e) => mb_strlen($e) > 10))->toBeFalse();
            expect($left->isRightAnd(fn ($r) => true))->toBeFalse();
        });
    ...
    describe('Edge Cases', function (): void {
        ...
        test('preserves Left through complex operations', function (): void {
            ...
            $result = $left
                ->map(fn ($x) => 'never')
                ->flatMap(fn ($x) => new Right('never'))
                ->filter(fn ($x) => true, 'never')
                ->mapLeft(fn (string $e) => mb_strtoupper($e));
        ...
        test('handles exception objects as Left value', function (): void {
            ...
            $mapped = $left->mapLeft(fn (RuntimeException $e) => $e->getMessage());
    ...
    describe('Regressions', function (): void {
        test('unwrapOrElse always calls callable with Left value', function (): void {
            ...
            $result = $left->unwrapOrElse(function ($e) use (&$receivedValue) {
                $receivedValue = $e;
        ...
        test('bimap only transforms Left value', function (): void {
            ...
            $result = $left->bimap(
                fn (int $l) => $l * 3,
                fn ($r) => throw new RuntimeException('Should not be called'),
            );
        ...
        test('isLeftAnd converts return value to boolean', function (): void {
            ...
            expect($left->isLeftAnd(fn ($e) => 0))->toBeFalse();
            expect($left->isLeftAnd(fn ($e) => 1))->toBeTrue();
            expect($left->isLeftAnd(fn ($e) => ''))->toBeFalse();
            expect($left->isLeftAnd(fn ($e) => 'truthy'))->toBeTrue();
            expect($left->isLeftAnd(fn ($e) => null))->toBeFalse();
        });
    });
});
