...
use Cline\Monad\Exceptions\TransposeExpectedSomeWithResultException;
use Cline\Monad\Exceptions\UnzipExpectedSomeWithTupleException;
...
abstract class Option implements IteratorAggregate
{
     ...
     */
    public static function fromValue(mixed $value, mixed $noneValue = null): self
    {
        if ($value === $noneValue) {
            return None::create();
        }
     ...
     */
    public static function fromArraysValue(mixed $array, int|string|null $key): self
    {
        if (null === $key) {
            return None::create();
        }
    ...
    {
        /** @phpstan-ignore return.type */
        return new LazyOption(static function () use ($callback, $arguments, $noneValue): None|Some {
            /** @var mixed */
     ...
     */
    public static function ensure(mixed $value, mixed $noneValue = null): LazyOption|self
    {
        ...
        if (is_callable($value)) {
            /** @phpstan-ignore return.type */
            return new LazyOption(static function () use ($value, $noneValue): self {
                /** @var mixed */
                $return = $value();
    ...
    {
        return static function (...$args) use ($callback, $noneValue): LazyOption|self {
            $reduced_args = array_reduce(
                $args,
                /** @param bool $status */
                /** @phpstan-ignore-next-line argument.type */
                static fn ($status, self $o) => $o->isEmpty() ? true : $status,
                false,
            );
    ...
    public function expect(string $message): mixed
    {
        return $this->unwrapOrThrow(
            CannotUnwrapNoneException::create($message),
        );
    }
     ...
     */
    public function zipWith(self $other, callable $f): self
    {
        if ($this->isDefined() && $other->isDefined()) {
    ...
    public function unzip(): array
    {
        ...
        throw UnzipExpectedSomeWithTupleException::create();
    }
     ...
     */
    public function unwrapOrDefault(): mixed
    {
    ...
    public function isNoneOr(callable $predicate): bool
    {
        ...
        return (bool) $predicate($this->get());
    }
     ...
     *
     * @return Result Transposed Result containing Option
     *
    ...
    public function transpose(): Result
    {
        ...
        throw TransposeExpectedSomeWithResultException::fromValue($value);
    }
     ...
     * @return Result<T, E> Ok with value if Some, Err with error if None
     */
     ...
     * @return Result<T, E> Ok with value if Some, Err with computed error if None
     */
     ...
     *
     * @throws HttpException When Option is None
     *
     * @return T The contained value
     */
    public function unwrapOrAbort(int $status = 404, ?string $message = null): mixed
    {
     ...
     * @param null|string           $message   Optional error message
     *
     * @throws HttpException When Option is None or condition fails
     *
     * @return T The contained value
     */
    public function unwrapOrAbortUnless(bool|callable $condition, int $status = 404, ?string $message = null): mixed
    {
        abort_unless($this->isDefined(), $status, $message ?? '');

        $value = $this->get();
        $ok = is_callable($condition) ? (bool) $condition($value) : $condition;
     ...
     */
    abstract public function unwrapOrElse(callable $callable): mixed;
     ...
     * @return T The contained value
     */
    abstract public function unwrapOrThrow(Exception $ex): mixed;
     ...
     */
    abstract public function orElse(callable $else): self;
    ...
    Use forAll() instead for side-effect operations
     ...
     */
    abstract public function forAll(callable $callable): self;
     ...
     */
    abstract public function map(callable $callable): self;
     ...
     */
    abstract public function filter(callable $callable): self;
     ...
     */
    abstract public function filterNot(callable $callable): self;
     ...
     */
    abstract public function select(mixed $value): self;
     ...
     */
    abstract public function reject(mixed $value): self;
     ...
     */
    abstract public function foldLeft(mixed $initialValue, callable $callable): mixed;
     ...
     * @return S                Final accumulated result
     */
    abstract public function foldRight(mixed $initialValue, callable $callable): mixed;
}
