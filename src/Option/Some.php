...
use ArrayIterator;
use Cline\Monad\Exceptions\FlatMapMustReturnOptionException;
use Deprecated;
use Exception;
...
final class Some extends Option
{
     ...
     * @return self<T>              This Some instance
     */
    public function orElse(callable $else): self
    {
        return $this;
    }
     ...
     */
    #[Deprecated(message: 'Use forAll() instead')]
     ...
     * @return self<T>           This Some instance for method chaining
     */
     ...
     */
    public function mapOrDefault(callable $f): mixed
    {
        return $f($this->value);
    }
     ...
     *
     * @throws FlatMapMustReturnOptionException When callable doesn't return an Option
     *
    ...
    public function flatMap(callable $callable): Option
    {
        ...
        throw_unless($rs instanceof Option, FlatMapMustReturnOptionException::create());
     ...
     * @return None|self<T>     This Some if predicate passes, None otherwise
     */
    public function filter(callable $callable): self|None
    {
     ...
     *
     * @param  callable(T):bool $callable Predicate function to test the value
     * @return None|self<T>     This Some if predicate fails, None otherwise
     */
    public function filterNot(callable $callable): self|None
    {
     ...
     * @return None|self<T> This Some if contains the value, None otherwise
     */
    public function select(mixed $value): self|None
    {
        if ($this->value === $value) {
            return $this;
        }
     ...
     * @return None|self<T> None if contains the value, this Some otherwise
     */
    public function reject(mixed $value): None|self
    {
        if ($this->value === $value) {
            return None::create();
        }
